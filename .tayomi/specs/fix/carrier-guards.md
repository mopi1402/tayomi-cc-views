...
- Narrowed on 2026-08-05, the prose half only: the guard now turns on whether the line below ANNOUNCES a payload shape, never on whether anything follows. Prose announces none, so it claims no line and is handed to no render, and a view spending nothing draws exactly as it does at the end of a message. The defect this constraint was written for is untouched, since it was never about drawing: it was about the sentence underneath disappearing, and prose is still on screen, now beneath the view rather than instead of it. What decides is the engine's own arity, `renderView` refusing a template that spends slots it receives no data for, so nothing here needs a table of which views are static. `@{view:hr}` with a report line glued under it is the case that asked for it, where a rule is furniture with no slot a payload could fill.
