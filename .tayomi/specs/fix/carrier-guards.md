...
- Protection is resolved BEFORE either carrier runs, and both read the same map of protected spans. `BLOCK_RE` must not match inside one, and `renderDecorated` must not test a line inside one.
- The decorator's zone is MEASURED before it is PARSED. The zone is the run of contiguous non-blank lines under the decorator line, whatever their shape; the parsers are then offered the zone and one of them claims it. A zone that is non-empty and claimed by nobody fails open, decorator line and payload left exactly as written.
- Consequently a STATIC view is summoned by a decorator whose zone is EMPTY: the decorator line is followed by a blank line, or it ends the message. A decorator followed directly by prose no longer summons anything, which is a behaviour change and is the point: today that prose is invisible to the scanner and the view renders as if the author had written nothing.
...
- `decorator.ts`'s ink test is REPLACED, not supplemented. Two guards asking two different questions about the same failure is how one of them rots.
...
- Replace `decorator.ts`'s ink test and extend `render.ts`'s guard with the one rule: a template that spends slots and resolved no field is hollow and its caller falls open. Delete the ink test rather than keeping both. (AC: 6)
