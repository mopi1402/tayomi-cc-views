...
Engines carrying the COMPOSITION go one further: the fleet answers each flush with one voice. Every zone is drawn by the engine that owns it, own templates and own colours, spliced into a single answer, so two engines' zones in one message can no longer interleave (the collage measured 2026-08-14, retired the same day). One engine from before the composition on the register sends the whole fleet back to the paragraphs above, collisions included, and the cure is updating that install: the session says so itself, a warning naming each outdated engine by version and path, once per session: printed at a fresh session start where the bookend is wired, and everywhere else (a resumed session, a fleet that mixed mid-session) handed to the model, which relays it in its reply (measured 2026-08-14: the prompt event prints nothing directly, and a resumed start prints under the replayed transcript). One edge worth knowing: a zone whose owner dies mid-message shows as written after a one-second wait, for the whole message, raw once being better than painted twice.
