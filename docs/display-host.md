...
**Where a registered tag resolves: in a view.** The engine runs no tag pass over the message, so your name is spent by a template file, and by your own `strict.failedLine` (the one host-authored string the engine inserts). A `{{brand}}` typed in the model's prose stays on screen as those nine characters. See "Only a template writes presentation" in `architecture.md` for why.
