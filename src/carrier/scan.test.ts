...
// The view the witnesses carry: this file is about the CARRIER, so the template is deliberately the most boring one
// that renders (no box, no frame, one loop).
...
// The fence, spelled here rather than imported from the carrier: a test that shares the production spelling of what it
// forbids on screen cannot catch a drift in it.
const FENCE = "```";
const HINT = `${FENCE}view:`;
/** One tick longer, which is how markdown quotes a fence inside a fence. */
const LONGER = FENCE + "`";

// The witness, character by character: 7 characters of prose, the 13-character opening fence line (7 to 19), the body,
// the closing fence.
const PROSE = "Note:\n\n";
const OPEN = `${HINT}note\n`;
const WITNESS = `${PROSE}${OPEN}note:\n- first\n- second\n${FENCE}\n`;

// The same message cut off before its block ever closes: 50 characters, of which the final flush used to show 7 and
// swallow 43.
const UNCLOSED = `${PROSE}${OPEN}note:\n- the block never closed`;

// A message carrying TWO blocks, the shape the report came in on. From the first complete `HINT` onwards the engine
// owns the screen for every later flush, so the second opening fence arrives with no host drawing to hide behind: it is
// the carrier's own business, cut wherever the stream cuts it.
...
// The flush counts the live stream produced for one message, kept as they were recorded: they cut the fence lines every
// way a stream can.
...
describe("a view block arriving on a stream", () => {
  it("shows no raw fence at ANY cut point of the message", () => {
    ...
    for (let at = 0; at <= WITNESS.length; at++) {
      ...
      // Ten cuts leaked before this held: 10 through 19, the opening fence line arriving one character at a time under
      // a scan that only knew complete lines.
      expect(screen).not.toContain(FENCE);
      // And what a flush shows is a PREFIX of what the message ends up showing: the cut delays content, it never drops
      // it and never has to take anything back.
...
describe("a message carrying two view blocks", () => {
  ...
  it("puts no raw fence on screen at any cut, and lands on the unchunked screen", () => {
    ...
    for (let at = 0; at <= TWO.length; at++) {
      // A flush whose text carries no view fence ANYWHERE is the host's own to draw, and this fix does not widen what
      // engages the engine: those cuts are not the carrier's to answer for. From the first complete hint on, they all
      // are.
...
describe("a block that never closes", () => {
  ...
  it("is still withheld on a flush that is not the last", () => {
    ...
    // A caller that omits the flag keeps that behaviour: the cut asks for `final` to be true, it never assumes it.
...
describe("cutUnclosedBlock", () => {
  ...
  it("hands back an ordinary code fence as soon as it cannot be a view opening", () => {
    // Three backticks alone could still become a view opening, so they wait one flush: withholding is always transient,
    // and the next flush settles it.
...
describe("what the block carrier must not render", () => {
  ...
  it("leaves a block QUOTED inside a longer fence exactly as written", () => {
    // What documentation about this package looks like. BLOCK_RE is a global regex with no notion of nesting, so the
    // example used to render itself and take the fences around it with it.
    const quoted = `${LONGER}\n${OPEN}note:\n- first\n${FENCE}\n${LONGER}\n`;
  ...
  it("shows an EMPTY block raw rather than rendering its skeleton", () => {
    // note.view spends a slot, so nothing arrived to fill it. The old guard only covered a NON-empty body, which left a
    // template drawing furniture around nothing looking like a successful render.
