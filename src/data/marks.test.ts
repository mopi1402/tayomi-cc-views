...
describe("the reserved control marks", () => {
  ...
  it("hold BOTH marks of a span, which is what puts them under every rule above", () => {
    // Enrolment is the whole test story for these two: out of the list they cost nothing
    // to leave a render, and none of the assertions here would look at them. They are
    // named as a PAIR because that is how they are written, and one enrolled without the
    // other would pass every rule above while the screen kept a stray code.
  ...
  it("come OFF a message's text, the one direction a code must never travel", () => {
    // "No message can type one" is a property of a keyboard, not of the channel: a
    // payload is JSON and JSON spells any code point. Since one of these resolves to a
    // style, a byte the model emitted would close a colour the template opened.
