...
// default, so a caller passing nothing gets a working engine. Distinct from
// DisplayHost (pipeline.ts), which carries host BEHAVIOUR; options carry host NAMES
// and sources. The palette is deliberately not here, see style.extendTags.

export interface RenderOptions {
  /**
   * Ordered template dirs, first hit wins. The ORDER is the caller's policy: a
   * consumer whose own templates must shadow the built-ins lists its dir first.
   * Default: defaultViewsPath() of template/load.ts.
   */
  viewsPath?: string[];
  /**
