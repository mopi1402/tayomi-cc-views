...
// Searched UPWARDS: the source tree, the compiled dist/ and an npm-installed copy sit at different depths under the
...
// The manifest check is what makes the answer OURS: a host that bundles the engine has its own views/ one hop above the
// bundle, and returning that directory is how the engine used to claim the host's templates as its own, silently.
...
export function bundledViewsDir(): string {
  ...
  // something a human can act on (docs/architecture/display-host.md).
...
/** Spelling a `.view` path is this module's job alone. */
export const viewFile = (dir: string, name: string): string => path.join(dir, name + VIEW_EXT);

/**
 * Every view a directory holds, sorted so a generated catalogue reads the same twice. A directory that does not exist
 * yields nothing rather than throwing: a search path routinely names one.
 */
