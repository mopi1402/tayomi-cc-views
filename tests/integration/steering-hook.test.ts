...
//
// What it exists to catch is a DELIVERY failure, which no other gate here can see. The text reaches a
// session through one narrow contract (`hookSpecificOutput.additionalContext` on stdout), and every way
// of getting it wrong is silent: the file moves and the hook says nothing, the note meant for a
// contributor rides along into the context, a missing file throws and takes the session start with it.
